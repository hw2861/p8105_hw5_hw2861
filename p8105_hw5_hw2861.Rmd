...
prop_function = function(city) {
 ...
 test= prop.test(
     x= city %>% pull(unsolved_homicides), 
     n= city %>% pull(total_homicides) )%>%
    broom::tidy() %>%
    select(estimate, conf.low,conf.high)
...
### First set the following design elements and generate 5000 datasets from the model given.
...
**Generate 5000 datasets from the model and repeat the above for different mu**
```{r }
sim_results_df = 
...
At effect sizes < 4, power increases as effect size increases. 
At effect sizes > 4, the power tends to stabilize at 1.
...
The sample average estimate of μ^ for which the null is rejected is  approximately equal to the true value when the the true μ >4. They have relatively large differences when the true value of 0< μ < 4. 